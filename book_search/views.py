...
def autocompleteauthor(request):
    if request.is_ajax():
        q = request.GET['term']
        drugs = Author.objects.filter(afull_name__icontains = q )[:20]
        results = []
        for drug in drugs:
            drug_json = {}
            drug_json['value'] = drug.afull_name
...
def autocompletepublisher(request):
    if request.is_ajax():
        q = request.GET['term']
        drugs = Edition.objects.filter(publisher__icontains = q )[:20]
        results = []
        for drug in drugs:
            drug_json = {}
            drug_json['value'] = drug.publisher
...
def add_book(request):
	...
	if request.method == 'GET':
		...
		try:
			if request.GET['is_fiction'] == 'on':
				ldb.insert_book(str(request.GET['title']), "true")
		except:
			ldb.insert_book(str(request.GET['title']), "false")
		

		ldb.insert_author(request.GET['af_name'], request.GET['al_name'], request.GET['af_name'] + ' ' + request.GET['al_name'], request.GET['pseudonym'])
		
		ldb.insert_book_author(Book.objects.first().book_id, Author.objects.first().author_id)

		ldb.insert_edition(request.GET['title'], request.GET['isbn'], request.GET['pub_date'], request.GET['publisher'], request.GET['language'])
		print "got heres12"
		ldb.insert_editor(request.GET['title'], request.GET['editor_name'])

		ldb.insert_genre(request.GET['title'], request.GET['genre'])
		results = {'msg':'#winning'}
		data = json.dumps(results)
    	mimetype = 'application/json'
    	return HttpResponse(data, mimetype)
